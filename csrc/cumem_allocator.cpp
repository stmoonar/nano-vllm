...
// environment variable NANOVLLM_ROCM_SLEEP_MEM_CHUNK_SIZE, specified in megabytes
...
static unsigned long long get_memcreate_chunk_size() {
  const char* env = getenv("NANOVLLM_ROCM_SLEEP_MEM_CHUNK_SIZE");
...
void* my_malloc(ssize_t size, int device, CUstream stream) {
  ...
  // ``NANOVLLM_ROCM_SLEEP_MEM_CHUNK_SIZE``; otherwise
